...
# Function to process user query with the new API
def process_query_with_api(query, completed_courses=None):
    # Call the API using the Gradio client
    response = client.predict(
        user_prompt={"text": query, "files": []},  # Replace with user's query
        api_name="/chat"
    )
    
    # Extract major or context based on the response
    major = extract_major_from_response(response)

    response_text = ""
    course_plan = []

    if major:
        # Fetch remaining courses or recommendations based on completed courses
        if completed_courses:
            course_plan = fetch_remaining_courses(major, completed_courses)
        else:
            course_plan = get_course_recommendations(major)
    ...
    # Combine API response with structured academic recommendations
    combined_text = f"API Insight:\n{response}\n\nPersonalized Course Plan:\n{response_text}"
    
    return combined_text, course_plan

# Extract the major or context based on the response
def extract_major_from_response(response):
    # List of possible majors to recognize from the response
    potential_majors = ["Computer Science", "Business", "Engineering", "Mathematics"]  # Extend as needed
    for major in potential_majors:
...
if __name__ == "__main__":
    ...
    # Generate course recommendations using the new API
    combined_text, course_plan = process_query_with_api(test_query, completed_courses=completed_courses)
    print(combined_text)
